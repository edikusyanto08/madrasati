                          ...
                          <tr>
                            ...
                            <td class="bolder">
                                ...
                                <?php if ($row['status'] == 'pending'): ?>
                                    <a href="<?php echo base_url();?>teacher/manage_online_exam_status/<?php echo $row['online_exam_id'];?>/published/<?php echo $data;?>/" onclick="return confirm('<?php echo get_phrase('confirm_publish');?>')" class="btn btn-info btn-sm"><?php echo get_phrase('publish');?></a><br>
                                <?php elseif ($row['status'] == 'published'): ?>
                                    <a href="<?php echo base_url();?>teacher/manage_online_exam_status/<?php echo $row['online_exam_id'];?>/expired/<?php echo $data;?>/" onclick="return confirm('Esta seguro que desea marcar como expirado el examen?')" class="btn btn-primary btn-sm"> <?php echo get_phrase('mark_as_expired');?></a><br>
                                <?php elseif($row['status'] == 'expired'): ?>
                                    <a href="#" class="btn btn-warning btn-sm"> <?php echo get_phrase('expired');?></a><br>
                                <?php endif; ?>
                                <a class="btn btn-danger btn-sm" onClick="return confirm('<?php echo get_phrase('confirm_delete');?>')" href="<?php echo base_url();?>teacher/manage_exams/delete/<?php echo $row['online_exam_id'];?>/<?php echo $data;?>/">Eliminar</a>
                            </td>
                          </tr>
                        <?php endforeach;?>
                      </tbody>
